...
const MiddlePart = React.memo(() => {
    ...
    const { isMobile, layout } = useResponsiveLayout();
    ...
    return (
        <div className={isMobile ? "px-4" : "px-20"}>
            {/* Stories Section */}
            <section>
                <div className={`flex items-centre rounded-b-md ${isMobile ? 'px-2' : ''}`}>
                    <div className="flex flex-row mr-4 cursor-pointer items-centre">
                        <Box sx={{ position: "relative", display: "inline-block" }}>
                            ...
                            >
                                {auth.user?.profile || auth.user?.name?.[0] || 'U'}
                            </Avatar>
                            <AddIcon
            ...
            {/* Create Post Section */}
            <Card className={`w-[100%] p-5 mt-5 ${isMobile ? 'mx-2' : ''}`}>
                ...
                <div className={`flex justify-center mt-5 ${isMobile ? 'space-x-4' : 'space-x-9'}`}>
                    <div className="flex items-center">
                        <IconButton color="primary" onClick={handleOpenCreatePostModal}>
                            <ImageIcon />
                        </IconButton>
                        <span style={{ fontSize: isMobile ? '12px' : '14px' }}>Photo</span>
                    </div>
                    <div className="flex items-center">
                        <IconButton color="primary" onClick={handleOpenCreatePostModal}>
                            <VideocamIcon />
            ...
            {/* Posts Feed */}
            <div className={`mt-5 ${isMobile ? 'space-y-2' : 'space-y-5'}`}>
                {memoizedPosts}
            </div>
